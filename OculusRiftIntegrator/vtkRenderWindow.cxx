...
void vtkRenderWindow::StereoMidpoint(void)
{
  ...
  if ((this->StereoType == VTK_STEREO_RED_BLUE) ||
	  ...
	  (this->StereoType == VTK_STEREO_OCULUSRIFT))
    {
    int *size;
    // get the size
    size = this->GetSize();
...
void vtkRenderWindow::StereoRenderComplete(void)
{
  switch (this->StereoType)
    ...
    case VTK_STEREO_OCULUSRIFT:
      {
      unsigned char *left, *leftTemp, *undistortedLeft, *right, *rightTemp, *undistortedRight;
      ...
      undistorted = this->GetPixelData(0, 0, size[0] - 1, size[1] - 1,
                                  !this->DoubleBuffer);                                  
      ...
      for (int y = 0; y <= (size[1] - 1); ++y)
        {
          for (int x = 1; x <= midX; ++x)
            {
            undistortedLeft = undistorted + (x * 3) + (y * size[0] * 3);
            leftTemp = sleft + ((x + viewportLeftEdge) * 3) + (y * size[0] * 3);
            
            *undistortedLeft++ = *leftTemp++;
            *undistortedLeft++ = *leftTemp++;
            *undistortedLeft++ = *leftTemp++;
            
            //Blank out original image
      ...
      for (int y = 0; y <= (size[1] - 1); ++y)
        {
          for (int x = 0; x < midX; ++x)
            {
            undistortedRight = undistorted + ((x + offsetX) * 3) + (y * size[0] * 3);
            rightTemp = sright + ((x + viewportLeftEdge) * 3) + (y * size[0] * 3);
            
            *undistortedRight++ = *rightTemp++;
            *undistortedRight++ = *rightTemp++;
            *undistortedRight++ = *rightTemp++;
            
            //Blank out original image
            right = sleft + ((x + offsetX) * 3) + (y * size[0] * 3);
            *right++ = 0;
            *right++ = 0;
            *right++ = 0;
            }
      ...
      for (int y = 0; y <= (size[1] - 1); ++y)
        {
          for (int x = 1; x <= midX; ++x)
            {
			      normalX = ((((float) x) / ((size[0] / 2.0) - 1)) * 2.0) - 1;
			      normalY = ( ((float) y) / (size[1] - 1) * 2.0) - 1;

			      radiusSquared = normalX*normalX + normalY*normalY;
			      //distortionScale = k0 + k1*radiusSquared + k2*radiusSquared*radiusSquared + k3*radiusSquared*radiusSquared*radiusSquared;
			      distortionScale = 1.0 + 0.22*radiusSquared + 0.24*radiusSquared*radiusSquared + 0*radiusSquared*radiusSquared*radiusSquared;				
      			
			      originalX = static_cast<int>( ((normalX * distortionScale) + 1) / 2 * (midX - 1) );
			      originalY = static_cast<int>( ((normalY * distortionScale) + 1) / 2 * (size[1] - 1) );
            
            //Assumes right eye viewport will be of the exact same size as the left eye viewport
            if (originalX >= 0 && originalY >= 0 && originalX < midX-1 && originalY < size[1]-1) 
              {
              // Left Eye
              left = sleft + ((x+45) * 3) + (y * size[0] * 3);
              ...
              *left++ = *leftTemp++;
              ...
              // Right Eye
              right = sleft + (((x-45) + offsetX) * 3) + (y * size[0] * 3);
