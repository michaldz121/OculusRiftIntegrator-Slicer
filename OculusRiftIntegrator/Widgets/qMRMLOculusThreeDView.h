...
#ifndef __qMRMLOculusThreeDView_h
#define __qMRMLOculusThreeDView_h
...
class qMRMLOculusThreeDViewPrivate;
class vtkMRMLScene;
...
/// batch process state.
/// \sa qMRMLOculusThreeDWidget, qMRMLOculusThreeDViewControllerWidget, qMRMLSliceView
class Q_SLICER_MODULE_OCULUSRIFTINTEGRATOR_WIDGETS_EXPORT qMRMLOculusThreeDView : public ctkVTKRenderView
{
  Q_OBJECT
public:
  /// Superclass typedef
  ...
  /// Constructors
  explicit qMRMLOculusThreeDView(QWidget* parent = 0);
  virtual ~qMRMLOculusThreeDView();
...
protected:
  QScopedPointer<qMRMLOculusThreeDViewPrivate> d_ptr;

private:
  Q_DECLARE_PRIVATE(qMRMLOculusThreeDView);
  Q_DISABLE_COPY(qMRMLOculusThreeDView);
};
